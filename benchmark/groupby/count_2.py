...
if __name__ == "__main__":
    ...
    distinct_sender = users.select("Sender").distinct().agg(count("*"))
    distinct_receiver = users.select("Receiver").distinct().agg(count("*"))
    distinct_puser = users.select("Puser").distinct().agg(count("*"))
    ...
    relation = relation.groupBy("Sender", "SenderAddress").agg(count("Receiver").alias("number of send to"))
