...
if __name__ == "__main__":
    ...
    emails = emails.select(
        "Sender",
        explode("Ccs").alias("Cc")
    )
    
    emails = emails.groupBy("Sender").agg(
        collect_set(col("Cc")).alias("CcSet"),
        collect_list(col("Cc")).alias("CcList"),
    )

    emails = emails.withColumn("CcSetLength", size(col("CcSet"))) \
                   .withColumn("CcListLength", size(col("CcList")))
