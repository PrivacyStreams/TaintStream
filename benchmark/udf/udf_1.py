...
if __name__ == "__main__":
    ...
    lit_udf = udf(lambda : 0)
    one_udf = udf(lambda x: x, StringType())
    two_udf = udf(lambda x, y: x+y, StringType())
    three_udf = udf(lambda x,y,z: x+y+z, StringType())

    emails = emails.withColumn("lit_udf", lit_udf()) \
                   .withColumn("one_udf", one_udf(col("Sender"))) \
                   .withColumn("two_udf", two_udf(col("Sender"), col("Receiver"))) \
                   .withColumn("three_udf", three_udf(col("Sender"), col("Receiver"), col("Subject")))
