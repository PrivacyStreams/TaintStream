...
if __name__ == "__main__":
    ...
    emails = emails.rdd.map(lambda x: ((x[0], x[1]), ([x[2]], 1))).reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1])).toDF(target_schema)
    emails = emails.select(
        col("col_1.Sender").alias("Sender"),
        col("col_1.Receiver").alias("Receiver"),
        col("col_2.SubjectList").alias("SubjectList"),
        col("col_2.EmailCount").alias("EmailCount"),
    )
